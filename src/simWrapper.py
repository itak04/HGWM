...
class SimWrapper:
    ...
    def get_path(self, path):
        ...
        if self.sim.pathfinder.find_path(path):
            ...
            # Instead of just logging and returning a large number,
            # provide more information and return a reasonable estimate
            
            # Check if this is a MultiGoalShortestPath
            if hasattr(path, 'requested_ends') and hasattr(path, 'requested_start'):
                start_pos = path.requested_start
                
                # Check if start position is navigable
                if not self.sim.pathfinder.is_navigable(start_pos):
                    logging.warning(f'Start position not navigable: {start_pos}')
                    # Try to snap to nearest navigable point
                    snapped_start = self.sim.pathfinder.snap_point(start_pos)
                    if self.sim.pathfinder.is_navigable(snapped_start):
                        path.requested_start = snapped_start
                        logging.info(f'Snapped start position to: {snapped_start}')
                        if self.sim.pathfinder.find_path(path):
                            return path.geodesic_distance
                
                # If we have multiple ends, check individual paths
                if hasattr(path, 'requested_ends') and len(path.requested_ends) > 0:
                    ends = path.requested_ends
                    ...
                    for i, end_pos in enumerate(ends):
                        if self.sim.pathfinder.is_navigable(end_pos):
                            # Check if start and end are on same navigation island
                            start_island = self.sim.pathfinder.get_island(start_pos)
                            end_island = self.sim.pathfinder.get_island(end_pos)
                            
                            if start_island == end_island:
                                # Try individual path
                                single_path = habitat_sim.ShortestPath()
                                single_path.requested_start = start_pos
                                single_path.requested_end = end_pos
                                
                                if self.sim.pathfinder.find_path(single_path):
                                    min_distance = min(min_distance, single_path.geodesic_distance)
                                    successful_paths += 1
                            else:
                                # Different islands - use Euclidean distance as estimate
                                euclidean_dist = np.linalg.norm(start_pos - end_pos)
                                min_distance = min(min_distance, euclidean_dist * 1.5)  # Add 50% penalty for non-direct path
                    
                    if successful_paths > 0:
                        logging.info(f'Found {successful_paths} reachable goals, min distance: {min_distance:.2f}')
                        return min_distance
                    else:
                        # No reachable goals, return euclidean distance to nearest goal
                        if len(ends) > 0:
                            distances = [np.linalg.norm(start_pos - end_pos) for end_pos in ends]
                            min_euclidean = min(distances)
                            logging.warning(f'No reachable goals found. Navigation islands issue. Using euclidean estimate: {min_euclidean:.2f}')
                            return min_euclidean * 2.0  # Penalty for unreachable
            
            # Fallback for other path types or if above fails
            logging.info('NO PATH FOUND - using fallback distance')
            return 1000
