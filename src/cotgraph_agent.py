...
class CoTGraphAgent(WMNavAgent):
    ...
    def _construct_goal_subgraph_via_llm(self, goal: str) -> Dict:
        ...
        try:
            ...
            subgraph_prompt = (
                ...
                f"(1) Primary room: The most likely room type where {goal} is typically found\n"
                f"(2) Secondary rooms: 1-2 alternative room types where {goal} might also be located\n"
                f"(3) Adjacent zones: Connected areas that provide visual access or pathways to target rooms\n\n"
                
                "2. PATHWAY and CONNECTIVITY MAPPING:\n"
                f"(1) Hallway connections: Common routes to reach target rooms from main areas\n"
                f"(2) Visual access points: Doorways/openings that help identify target rooms\n"
                f"(3) Navigation landmarks: Key reference points for wayfinding\n\n"
                
                "3. TARGET-ORIENTED SPATIAL ANCHORS and NAVIGATION CUES:\n"
                f"(1) Directional anchors: 3-4 objects that DIRECTLY indicate where {goal} is likely located (not just co-occurring objects)\n"
                f"(2) Visual pathways: Objects that form visual lines leading toward typical {goal} placement areas\n"
                f"(3) Distance indicators: Objects that help estimate proximity to {goal} based on typical room layouts\n"
                f"(4) Positioning cues: Furniture/fixtures that indicate the specific area within a room where {goal} is usually found\n\n"
                
                "4. SYSTEMATIC EXPLORATION APPROACH:\n"
                f"(1) Exploration priorities: Which areas to check first when searching\n"
                f"(2) Recovery strategies: What to do when initial search paths fail\n\n"
    ...
    def _vlm_enhanced_panoramic_analysis(self, image, goal: str, goal_subgraph: Dict) -> Dict:
        ...
        evaluator_prompt = (
            ...
            f"   (a) Only if the {goal} is found in image, assign a score of 10.\n"
            f"   (b) If the path is a dead end and it is clear that the target is not in sight, assign a score of 0.\n"
            f"   (c) If no goal-related objects are visible but the view leads to an unexplored area (clear turn, open door, hallway), However, if you see objects related to the {goal}, first infer the current room type, then assign a score 1-9 based on how strongly these objects suggest the {goal} can be found nearby. assign a score reflecting your common-sense estimate of finding the {goal} there.Note: a chair must have a backrest and is not a stool; a chair is not a sofa (couch), and a sofa is not a bed.\n"
            f"   (d) Treat any clear turn, open door, or unobstructed hallway as an opportunity to explore further—score higher for more promising paths.\n\n"
            f"Json Format:\n"
